              ...
              {recentActivities.map((activity) => (
                <div key={activity.id} className="flex items-start gap-4 p-4 rounded-lg hover:bg-gray-50">
                  <div className={`w-2 h-2 rounded-full mt-2 ${
                    ...
                    activity.status === 'shortlisted' ? 'bg-green-500' :
                    activity.status === 'interview' ? 'bg-purple-500' :
                    'bg-red-500'
                  }`}></div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h3 className="font-medium text-gray-900">{activity.candidate}</h3>
          ...
          <div className="p-6">
            <div className="space-y-4">
              {upcomingInterviews.map((interview) => (
                <div key={interview.id} className="p-4 rounded-lg border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-orange-800 mt-1">
              You have 15 pending candidate reviews and 3 leave applications awaiting approval.
            </p>
          </div>
        </div>
      </div>
