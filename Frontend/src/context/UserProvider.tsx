...
//     try {
//       const res = await axios.put(`${API_BASE_URL}/users/${id}`, userData);
//       setUsers((prev) =>
...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchLeaves = async (userId: string) => {
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/leaves/user/${userId}`);
      setLeaves(res.data);
    } catch (err) {
  ...
  const applyLeave = async (leaveData: LeaveFormData) => {
    try {
      const res = await axios.post(`${API_BASE_URL}/leaves/apply`, leaveData);
  ...
  const updateLeaveStatus = async (
    ...
    try {
      const res = await axios.put(`${API_BASE_URL}/leaves/${id}`, { status });
      setLeaves((prev) =>
